...
$(function() {
	...
	$("#reportType").change(function(){
		if($(this).val() == "mReport"){
			...
			$( "#startDate" ).val("<%=month %>");
			$( "#endDate" ).val("<%=month %>");
			$("#reportfile").val("/dlszjgj/cash_collect_trade_m.raq");
			...
			$( "#startDate" ).val("<%=today %>");
			$( "#endDate" ).val("<%=today %>");
			$("#reportfile").val("/dlszjgj/cash_collect_trade_d.raq");
		}
	});
...
<body>
	...
	<div id="reportContent"><img alt="#" src="images/magnifier.gif">&nbsp;&nbsp;代理商资金归集交易量统计报表</div>
	<form action="${pageContext.request.contextPath}/dlszjgj_report_.action" method="post" target="reportFrame">
	<input id = "reportfile" type="hidden" name="reportCondis.reportName" value="/dlszjgj/cash_collect_trade_d.raq"/>
