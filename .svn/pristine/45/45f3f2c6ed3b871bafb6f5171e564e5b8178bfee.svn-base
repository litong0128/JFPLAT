...
String reportTitle = "MISPOS招联分期手续费收入报表" ;
%>  
...
$(function() {
	...
	var pre = "finaChannel";
	var text = "金融通道&nbsp;";
	var mUrl = "${pageContext.request.contextPath}/getData_finaChannelData.action?proflag=005"; //获取数据的url
	popBoxHtml(outObj, pre, text);
	popBoxAction(pre, mUrl);
	//下拉菜单生成
	...
	var reportfile_m = "/misposfq/mucfc_pos_installment_income_m.raq";//月报表文件
	var reportfile_d = "/misposfq/mucfc_pos_installment_income_d.raq";//日报表文件
...
<body>
	...
	<form action="${pageContext.request.contextPath}/misposfq_report_.action" method="post" target="reportFrame">
	<input id="reportfile" type="hidden" name="reportCondis.reportName" value="/misposfq/mucfc_pos_installment_income_d.raq"/>
