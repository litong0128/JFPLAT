...
$(function() {
	...
	var text = "业务场景&nbsp;";
	var mUrl = "${pageContext.request.contextPath}/getData_busiScenarioData.action?proflag=003"; //获取数据的url
	popBoxHtml(outObj, pre, text);
	popBoxAction(pre, mUrl);
	...
	var reportfile_m = "/zzzd/self_terminal_trade_m.raq";//月报表文件
	var reportfile_d = "/zzzd/self_terminal_trade_d.raq";//日报表文件
...
<body>
	...
	<form action="${pageContext.request.contextPath}/zzzd_report_.action" method="post" target="reportFrame">
	<input id="reportfile" type="hidden" name="reportCondis.reportName" value="/zzzd/self_terminal_trade_d.raq"/>
