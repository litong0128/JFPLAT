...
function popBoxHtml(outObj, pre, text){
	var html = '<div id="'+pre+'inputBox">'+
		...
		'<div id="'+pre+'set2" style="display:inline-block; border-left:1px solid; vertical-align:top; margin-left:5px;"></div>'+
		'<div id="'+pre+'set3" style="display:inline-block; border-left:1px solid; vertical-align:top; margin-left:5px;"></div>'+
		'<div id="'+pre+'set4" style="display:inline-block; border-left:1px solid; vertical-align:top; margin-left:5px;"></div>'+
		'</div>'+
		'<hr>'+
...
function popBoxAction(pre, mUrl){
	...
	all1.click(function(){
		clickAllDisable(all1, all2, eName, search, tempArea);
		if(all1.prop("checked") == true){ //all1,all2只选一个
			all2.prop("checked", false);
		}
	});
	all2.click(function(){
		clickAllDisable(all1, all2, eName, search, tempArea);
		if(all2.prop("checked") == true){ //all1,all2只选一个
		...
		success:function(data){
			...
			$.each(data, function(n, value){
				...
				if(all1 != "" && all2 != "" && (all1.prop("checked") == true || all2.prop("checked") == true)){
					checkBox = "<input type='checkbox' disabled='true' id='"+eName+n+"' name='"+eName+"' value="+value.id+" onClick='clickTempStore(\""+pre+"\", this);'><label for='"+eName+n+"'>"+value.id+"-"+value.name+"</label><br>";
//					checkBox = "<input type='checkbox' disabled='true' id='"+eName+n+"' name='"+eName+"' value="+value.id+"><label for='"+eName+n+"'>"+value.id+"-"+value.name+"</label><br>";
					...
					if($.inArray(value.id+"", curTemp) != -1){
						checkBox = "<input type='checkbox' checked='true' id='"+eName+n+"' name='"+eName+"' value="+value.id+" onClick='clickTempStore(\""+pre+"\", this);'><label for='"+eName+n+"'>"+value.id+"-"+value.name+"</label><br>";
//						checkBox = "<input type='checkbox' checked='true' id='"+eName+n+"' name='"+eName+"' value="+value.id+"><label for='"+eName+n+"'>"+value.id+"-"+value.name+"</label><br>";
						...
						checkBox = "<input type='checkbox' id='"+eName+n+"' name='"+eName+"' value="+value.id+" onClick='clickTempStore(\""+pre+"\", this);'><label for='"+eName+n+"'>"+value.id+"-"+value.name+"</label><br>";
...
function searchTime(search, mUrl, mDivs, eName, pre){
	search.keyup(function(event) {
		...
		if(keyCode > 40 || keyCode == 8 || keyCode ==32) {
			//获取暂存区域的元素id
			var curTemp = getValueFromTempArea(pre);
			loadDatas(mUrl, $(this).val(), mDivs, eName, "", "", curTemp, pre);
			//alert($(this).val());
		}
	});
}
...
function setCheckedValues(all1, all2, checkBoxName, displayBox, valueBox, tempArea){
	...
	if(all1.prop("checked") == true){
		chkDisplay.push(all1.next().text());
		chkValue.push(all1.val());
	}else if(all2.prop("checked") == true){
		chkDisplay.push(all2.next().text());
		chkValue.push(all2.val());
	}else{
//		$("input[name='"+checkBoxName+"']:checked").each(function(){
//			chkDisplay.push($(this).next().text());
...
//	}else{
//		//两个都不为空时，需要去除重复的选择
//		var curVbv = valueBox.val().split(",");
//		var curDbv = displayBox.val().split(",");
//		if((chkValue[0] == "" || chkValue[0] == "all") || (curVbv[0] == "" || curVbv[0] == "all")){
//			vbv = chkValue;
//			dbv = chkDisplay;
//		}else{
//			//当前的+追加的  拼接排序
...
//			var i = 0;
//			var j = 0;
//			while(i < curVbv.length && j < chkValue.length){
//				if(curVbv[i] - chkValue[j] < 0){
//					i++;
//				}else if(curVbv[i] - chkValue[j] > 0){
//					curVbv.splice(i, 0, chkValue[j]); //i的前面添加j
//					curDbv.splice(i, 0, chkDisplay[j]); //i的前面添加j
//					i++;
//					j++;
//				}else{
//					j++;
//				}
//			}
//			while(j < chkValue.length){
//				curVbv.push(chkValue[j]);
//				curDbv.push(chkDisplay[j]);
//				j++;
//			}
//			vbv = curVbv;
//			dbv = curDbv;
...
//点击all禁用或启用所有商户的复选框checkbox
function clickAllDisable(all1, all2, checkBoxName, search, tempArea){
	//alert(all1.prop("checked")+" "+all2.prop("checked"));
	//取消已选择的
	$("input[name='"+checkBoxName+"']:checked").each(function(){
		$(this).prop("checked", false);
	});
	//禁用或者启用
	if(all1.prop("checked") == true || all2.prop("checked") == true ){
...
function clickTempStore(pre, check){
	...
	var temp = '<div id="'+checkValue+'" class="aa_cont apadd_suc"><span>'+checkObj.next().text()+'</span></div>';
	var tempArea = $("#"+pre+"tempArea"); //暂存区域
