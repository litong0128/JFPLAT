...
$(function() {
	...
	$("#reportType").change(function(){
		if($(this).val() == "mReport"){
			...
			$( "#startDate" ).val("<%=month %>");
			$( "#endDate" ).val("<%=month %>");
			$("#reportfile").val("/ctzz/user_withdraw_m.raq");
			...
			$( "#startDate" ).val("<%=today %>");
			$( "#endDate" ).val("<%=today %>");
			$("#reportfile").val("/ctzz/user_withdraw_d.raq");
		}
	});
...
<body>
	...
	<div id="reportContent"><img alt="#" src="images/magnifier.gif">&nbsp;&nbsp;用户提现报表</div>
	<form action="${pageContext.request.contextPath}/ctzz_report_.action" method="post" target="reportFrame">
	<input type="hidden" id = "reportfile" name="reportCondis.reportName" value="/ctzz/user_withdraw_d.raq"/>
